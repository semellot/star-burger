...
Доступен по адресу [semellot.one](https://semellot.one/)
...
Определите переменные окружения `SECRET_KEY` и `YANDEX_APIKEY`.
`YANDEX_APIKEY` - можно получить в кабинете разработчика Яндекс по ссылке https://developer.tech.yandex.ru/services/.
